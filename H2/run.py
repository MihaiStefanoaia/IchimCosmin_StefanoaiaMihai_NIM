            ...
            for inertia in np.arange(0.7, 1, 0.05):
                for self_bias in np.arange(0.1, 0.5, 0.1):
                    for global_bias in np.arange(0.05, 0.3, 0.05):
                        for exploration_factor in np.arange(0, 0.015, 0.003):
                            for adjust_factor in re_spread:
                                for i in range(3):
                                    dimensions = 10
                                    params.append((function,adjust_factor,inertia,self_bias,global_bias,exploration_factor,30,250,5000,dimensions,tolerances[function][dimensions],torch.device('cuda'),False,10))
        runlist = []
        for run in params:
